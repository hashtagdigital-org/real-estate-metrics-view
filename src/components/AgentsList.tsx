      ...
      {agents.map((agent) => (
        <Card key={agent.id} className="border shadow-sm overflow-hidden">
          <CardContent className="p-0">
            <div className="p-3 md:p-4 flex flex-col md:flex-row gap-3 md:gap-4 md:items-center justify-between">
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs md:text-sm text-gray-500">{agent.role}</p>
                </div>
              </div>
              
              <div className="flex flex-wrap gap-1 md:gap-2 mt-2 md:mt-0">
                <Badge variant="outline" className={`text-xs md:text-sm ${getStatusColor('conversionRate', agent.performance.conversionRate)}`}>
                  {agent.performance.conversionRate}% Conv.
                </Badge>
                <Badge variant="outline" className={`text-xs md:text-sm ${getStatusColor('responseTime', agent.performance.responseTime)}`}>
                  {agent.performance.responseTime}h Resp.
                </Badge>
              </div>
            </div>
            
            <div className="grid grid-cols-5 bg-gray-50 border-t text-xs md:text-sm">
              <div className="p-2 md:p-3 text-center border-r">
            ...
            <div className="p-3 md:p-4 bg-white border-t text-right">
              <Button 
